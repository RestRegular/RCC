...
void LLVMCodeGenVisitor::printIR() const {
...
llvm::Type *LLVMCodeGenVisitor::getLLVMType(
    const std::shared_ptr<symbol::TypeLabelSymbol> &typeLabel) {
    // 动态类型语言：统一使用 opaque pointer
    return getValueType();
}

llvm::Type *LLVMCodeGenVisitor::getIntType(unsigned bits) const {
    return llvm::Type::getIntNTy(*TheContext, bits);
}

llvm::Type *LLVMCodeGenVisitor::getVoidType() const {
...
void LLVMCodeGenVisitor::storeVariable(const std::string &name,
    llvm::Value *value) {
...
llvm::Value *LLVMCodeGenVisitor::createGlobalStringPtr(const std::string &str) {
...
llvm::BasicBlock *LLVMCodeGenVisitor::getCurrentBlock() const {
    ...
    if (value->getType()->isPointerTy()) {
        return Builder->CreateICmpNE(
            value, llvm::ConstantPointerNull::get(
                llvm::cast<llvm::PointerType>(value->getType())), "tobool");
    }
    // 默认：假设非 null 为 true
    return Builder->CreateICmpNE(
        value, llvm::Constant::getNullValue(value->getType()), "tobool");
}

llvm::Function *LLVMCodeGenVisitor::getOrCreateExternalFunc(
    const std::string &name, llvm::FunctionType *funcType) {
...
void LLVMCodeGenVisitor::enableDebug(bool enable) {
    DebugMode = enable;
}
...
void LLVMCodeGenVisitor::reset() {
    ...
    while (!BreakTargetStack.empty()) BreakTargetStack.pop();
    while (!ContinueTargetStack.empty()) ContinueTargetStack.pop();
    while (!LoopCondBBStack.empty()) LoopCondBBStack.pop();
...
void LLVMCodeGenVisitor::visitIntegerLiteralNode(IntegerLiteralNode &node) {
    ...
    auto *intConst = llvm::ConstantInt::get(
        llvm::Type::getInt64Ty(*TheContext), intVal);
...
void LLVMCodeGenVisitor::visitFloatLiteralNode(FloatLiteralNode &node) {
    ...
    auto *intConst = llvm::ConstantInt::get(
        llvm::Type::getInt64Ty(*TheContext), bits);
    auto *ptrVal = llvm::ConstantExpr::getIntToPtr(intConst, getValueType());
...
void LLVMCodeGenVisitor::visitBooleanLiteralNode(BooleanLiteralNode &node) {
    ...
    auto *intConst = llvm::ConstantInt::get(
        llvm::Type::getInt64Ty(*TheContext), boolVal ? 1 : 0);
    auto *ptrVal = llvm::ConstantExpr::getIntToPtr(intConst, getValueType());
...
void LLVMCodeGenVisitor::visitCharacterLiteralNode(CharacterLiteralNode &node) {
    ...
    auto *intConst = llvm::ConstantInt::get(
        llvm::Type::getInt64Ty(*TheContext), static_cast<int64_t>(charVal));
    auto *ptrVal = llvm::ConstantExpr::getIntToPtr(intConst, getValueType());
...
void LLVMCodeGenVisitor::visitVariableDefinitionNode(VariableDefinitionNode &node) {
    ...
    for (const auto &varDef : node.getVarDefs()) {
        ...
        auto *alloca = createEntryBlockAlloca(
            CurrentFunction, varName, getValueType());

        // 初始化为 null
        Builder->CreateStore(
            llvm::ConstantPointerNull::get(getValueType()), alloca);
...
void LLVMCodeGenVisitor::visitFunctionCallNode(FunctionCallNode &node) {
    ...
    if (argsNode) {
        ...
        if (auto *blockRanger = dynamic_cast<BlockRangerNode*>(argsNode.get())) {
            for (const auto &expr : blockRanger->getBodyExpressions()) {
                ...
                argValues.push_back(
                    argVal ? argVal : llvm::ConstantPointerNull::get(getValueType()));
            }
    ...
    if (callee) {
        ...
        LLVM_DEBUG("FunctionCallNode: " << funcName
            << " (" << argValues.size() << " args)");
        pushValue(callInst);
    } else {
        // 未找到函数：声明为外部函数并调用
...
void LLVMCodeGenVisitor::visitProgramNode(ProgramNode &node) {
    ...
    auto *mainType = llvm::FunctionType::get(
        llvm::Type::getInt32Ty(*TheContext),
        {llvm::Type::getInt32Ty(*TheContext), getValueType()},
        false);
    ...
    case NodeType::LOGICAL:
        {
            ...
            if (opStr == "&&") {
                boolResult = Builder->CreateAnd(
                    coerceToBool(left), coerceToBool(right), "land");
            } else if (opStr == "||") {
                boolResult = Builder->CreateOr(
                    coerceToBool(left), coerceToBool(right), "lor");
            } else {
...
void LLVMCodeGenVisitor::handleLogicalShortCircuit(InfixExpressionNode &node) {
    ...
    // 创建基本块
    llvm::BasicBlock *evalRightBB = llvm::BasicBlock::Create(
        *TheContext, "eval.right", func);
...
void LLVMCodeGenVisitor::visitPrefixExpressionNode(PrefixExpressionNode &node) {
    ...
    if (prefixType == NodeType::NOT) {
        ...
        logUnimplemented("PrefixExpressionNode (type: " +
            std::to_string(static_cast<int>(prefixType)) + ")");
        node.getNode()->acceptVisitor(*this);
    }
}
...
void LLVMCodeGenVisitor::visitPostfixNode(PostfixExpressionNode &node) {
    ...
    if (targetNode->getRealType() == NodeType::IDENTIFIER) {
        ...
        if (postfixType == NodeType::INCREMENT) {
            ...
            auto *newInt = Builder->CreateAdd(oldInt,
                llvm::ConstantInt::get(int64Ty, 1), "post.inc.new");
            ...
            auto *newInt = Builder->CreateSub(oldInt,
                llvm::ConstantInt::get(int64Ty, 1), "post.dec.new");
            auto *newPtr = Builder->CreateIntToPtr(newInt, getValueType());
            storeVariable(name, newPtr);
            pushValue(oldVal); // 后缀返回旧值
            LLVM_DEBUG("PostfixNode: " << name << "--");
        } else {
            logUnimplemented("PostfixNode (type: " +
                std::to_string(static_cast<int>(postfixType)) + ")");
...
void LLVMCodeGenVisitor::visitFunctionDefinitionNode(FunctionDefinitionNode &node) {
        ...
        if (auto *parenRanger = dynamic_cast<ParenRangerNode*>(argsNode.get())) {
            if (parenRanger->getRangerNode()) {
                ...
                if (parenRanger->getRangerNode()->getRealType() == NodeType::PARALLEL) {
                    ...
                    auto *id = static_cast<IdentifierNode*>(
                        parenRanger->getRangerNode().get());
                    paramNames.push_back(id->getName());
                }
            }
    ...
    LLVM_DEBUG("FunctionDefinitionNode: " << funcName
        << " (" << paramNames.size() << " params) defined");
}
    ...
    for (size_t i = 0; i < branches.size(); i++) {
        ...
        if (condExpr) {
            ...
            auto *thenBB = llvm::BasicBlock::Create(
                *TheContext, "cond.then." + std::to_string(i), func);
            auto *nextBB = llvm::BasicBlock::Create(
                *TheContext, "cond.next." + std::to_string(i), func);
...
void LLVMCodeGenVisitor::visitReturnExpressionNode(ReturnExpressionNode &node) {
    ...
    if (node.getReturnNode()) {
        ...
        Builder->CreateRet(
            retVal ? retVal : llvm::ConstantPointerNull::get(getValueType()));
    } else {
        Builder->CreateRet(llvm::ConstantPointerNull::get(getValueType()));
    }
...
void LLVMCodeGenVisitor::visitBreakExpressionNode(BreakExpressionNode &node) {
    if (!BreakTargetStack.empty()) {
        ...
        auto *unreachableBB = llvm::BasicBlock::Create(
            *TheContext, "break.unreachable", func);
