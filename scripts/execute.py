...
def add_version_to_executable(executable_path, version):
    """
    在可执行文件的文件名后添加版本号

    参数:
        executable_path: 原始可执行文件的路径
        version: 要添加的版本号，如 "1.0.0" 或 "beta"
    ...
    # 在文件名后添加版本号，例如 "app.exe" 变成 "app_v1.0.0.exe"
    new_base_name = f"{base_name}-v{version}"
...
def save_project_version_release(version: dict):
    ...
    try:
        ...
        shutil.copy2(executable_path, add_version_to_executable(executable_path, f"{version['MAJOR']}.{version['MINOR']}.{version['PATCH']}"))
